...
const BASE_URL_ONECALL = `https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&exclude=minutely&appid=${APIKEY}&lang=es&units=metric`;
...
export async function getCurrent () {
  const response = await fetch(BASE_URL_ONECALL)
  const data = await response.json()
  return data
}